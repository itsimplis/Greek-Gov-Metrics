...
@Component({
  selector: 'app-expenses-region',
  templateUrl: './expenses-region.component.html',
  styleUrls: ['./expenses-region.component.scss']
})

export class ExpensesRegionComponent implements OnInit {
  kaes: any[];
  municipalities: any[];
  regions: any[];
  ...
  expensesDate: any[];
  selectedRegionDate: string = "";
  selectedRegionThematic: string = "";
  selectedRegionThematicLabel: string = "";
  selectedRegionKae: string = "";
  selectedRegionKaeLabel: string = "";
  ...
  ngOnInit(): void {
    this.sharedService.expensesPage = "Περιφερειών";

    this.apiService.getRegionsOfDecisions().subscribe({
      next: (data) => {
        this.regions = data;

        this.route.paramMap.subscribe(params => {

          let regionIdentifier: string | null;

          const regionUid = params.get('regionUid');
          if (regionUid) {
            regionIdentifier = regionUid;
          } else {
            const regionLabel = params.get('regionLabel');
            if (regionLabel) {
              regionIdentifier = this.regions.find(item => item.label === regionLabel).uid;
            }
          }
          ...
          if (tabButton) {
            ...
            if (activeTabId == "expenses-per-thematic-tab") {
              this.selectedRegionThematic = regionIdentifier!;
              this.onRegionThematicChanged(this.selectedRegionThematic);
            } else {
              this.selectedRegionKae = regionIdentifier!;
              this.onRegionKaeChanged(this.selectedRegionKae);
            }
          }
        });
      },
  ...
  pieChartOptionsThematic = {
    title: 'Έξοδα ανά θεματική κατηγορία  (δεδομένα από το 2019)',
    width: 1000,
    height: 600,
  ...
  pieChartOptionsKae = {
    title: 'Έξοδα ανά KAE  (δεδομένα από το 2019)',
    width: 1000,
    height: 600,
    sliceVisibilityThreshold: 0
  };
  ...
  // Call to the API to get the thematic categories that exist in decisions for the selected region
  private loadThematicCategoriesPerRegion(selReg: string,): void {
    this.thematicCategories = [];

    this.apiService.getThematicCategoriesPerRegion(selReg).subscribe({
      next: (data) => {
  ...
  // Call to the API to get the kaes that exist in decisions for the selected region
  private loadKaesPerRegion(selReg: string,): void {
    this.kaes = [];

    this.apiService.getKaesPerRegion(selReg).subscribe({
      next: (data) => {
        this.kaes = data;
      },
      error: (error) => {
  ...
  // Call to the API to get the expenses per region per thematic category
  private loadExpensesPerRegionPerThematic(selReg: string, selThem: string): void {
    this.sharedService.startRequest();
    this.expensesPerThematic = [];

    this.apiService.getRegionExpensesPerThematic(selReg, selThem).subscribe({
      next: (data) => {
        ...
        this.scrollToTable("region-thematic-table");
      },
      error: (error) => {
        console.log(error);
        this.sharedService.endRequest();
  ...
  // Call to the API to get the sum of expenses for each Thematic category, per region
  private loadExpensesPerRegionAllThematic(selReg: string): void {
    this.sharedService.startRequest();
    this.expensesAllThematic = [];

    this.apiService.getRegionExpensesAllThematic(selReg).subscribe({
      next: (data) => {
        this.expensesAllThematic = data;
        this.pieChartDataThematic = this.expensesAllThematic.map(item => [item.thematic_category, item.sum_amount || 0, item.thematic_category_uid]);
        this.sharedService.endRequest();
      },
      error: (error) => {
        console.log(error);
  ...
  private loadRegionThematicExpensesPerYear(selReg: string): void {
    this.sharedService.startRequest();
    this.expensesAllThematicYearly = [];

    this.apiService.getRegionThematicExpensesPerYear(selReg).subscribe({
      next: (data) => {
        this.expensesAllThematicYearly = data;
        this.lineChartDataThematic = this.expensesAllThematicYearly.map(item => [item.year, item.total_expenses]);
        this.sharedService.endRequest();
      },
      error: (error) => {
        console.log(error);
  ...
  private loadRegionKaeExpensesPerYear(selReg: string): void {
    this.sharedService.startRequest();
    this.expensesAllKaeYearly = [];

    this.apiService.getRegionKaeExpensesPerYear(selReg).subscribe({
      next: (data) => {
        this.expensesAllKaeYearly = data;
        this.lineChartDataKae = this.expensesAllKaeYearly.map(item => [item.year, item.total_expenses]);
        this.sharedService.endRequest();
      },
      error: (error) => {
        console.log(error);
  ...
  // Call to the API to get the expenses per region per kae
  private loadExpensesPerRegionPerKae(selReg: string, selKae: string): void {
    this.sharedService.startRequest();
    this.expensesPerKae = [];

    this.resetFilters("kae");
    this.apiService.getRegionExpensesPerKae(selReg, selKae).subscribe({
      next: (data) => {
        this.currentPageKae = 1;
        this.expensesPerKae = data;
        this.expensesPerKaeFiltered = [...data];
        this.sharedService.endRequest();
        this.scrollToTable("region-kae-table");
      },
      error: (error) => {
        console.log(error);
        this.sharedService.endRequest();
  ...
  // Call to the API to get the sum of expenses for each Kae, per region
  private loadExpensesPerRegionAllKae(selReg: string): void {
    this.sharedService.startRequest();
    this.expensesAllKae = [];

    this.apiService.getRegionExpensesAllKae(selReg).subscribe({
      next: (data) => {
        this.expensesAllKae = data;
        this.pieChartDataKae = this.expensesAllKae.map(item => [item.kae, item.sum_amount || 0, item.kae_uid]);
        this.sharedService.endRequest();
      },
      error: (error) => {
        console.log(error);
  ...
  exportTableToExcel(table_id: string, arg1: string, arg2: string, fullTable: boolean): void {
    ...
    if (fullTable) {
      if (table_id == "region-thematic-table") {
        const keys = Object.keys(this.columnsThematic).filter(key => this.columnsThematic[key as keyof ColumnsThematic].visible);
        const headers = keys.map(key => this.columnsThematic[key as keyof ColumnsThematic].header);
        dataSource.push(headers);
        dataSource.push(...this.expensesPerThematicFiltered.map(item => {
          return keys.map(key => {
            if (key === 'suspicious') {
        ...
        const keys = Object.keys(this.columnsKae).filter(key => this.columnsKae[key as keyof ColumnsKae].visible);
        const headers = keys.map(key => this.columnsKae[key as keyof ColumnsKae].header);
        dataSource.push(headers);
        dataSource.push(...this.expensesPerKaeFiltered.map(item => {
          return keys.map(key => {
            if (key === 'suspicious') {
              return item[key as keyof typeof item] === 1 ? 'Οχι' : 'Ναι';
            } else {
  ...
  // Call to the API to get the expenses per region per date
  private loadExpensesPerRegionPerDate(selReg: string, selDate: string): void {
    // TODO: ...
  }

  onRegionThematicChanged(event: any) {
    this.selectedThematic = '';
    this.expensesPerThematic = [];

    if (this.selectedRegionThematic != '') {
      this.loadThematicCategoriesPerRegion(this.selectedRegionThematic);
      this.loadExpensesPerRegionAllThematic(this.selectedRegionThematic);
      this.loadRegionThematicExpensesPerYear(this.selectedRegionThematic);
      this.selectedRegionThematicLabel = this.regions.find(item => item.uid === event).label;
    } else {
      this.selectedRegionThematicLabel = '';
      this.pieChartDataThematic = [];
      this.lineChartDataThematic = [];
    }
  }

  onRegionKaeChanged(event: any) {
    this.selectedKae = '';
    this.expensesPerKae = [];

    if (this.selectedRegionKae != '') {
      this.loadExpensesPerRegionAllKae(this.selectedRegionKae);
      this.loadKaesPerRegion(this.selectedRegionKae);
      this.loadRegionKaeExpensesPerYear(this.selectedRegionKae);
      this.selectedRegionKaeLabel = this.regions.find(item => item.uid === event).label;
    } else {
      this.selectedRegionKaeLabel = '';
      this.pieChartDataKae = [];
      this.lineChartDataKae = [];
    }
  }
  ...
  onThematicChanged(event: any) {
    if (this.selectedThematic != '') {
      this.loadExpensesPerRegionPerThematic(this.selectedRegionThematic, this.selectedThematic);
  ...
  onKaeChanged(event: any) {
    if (this.selectedKae != '') {
      this.loadExpensesPerRegionPerKae(this.selectedRegionKae, this.selectedKae);
  ...
  onPieChartThematicMouseOver(event: ChartMouseOverEvent) {
    const chartElement: HTMLElement | null = document.querySelector('.chart-selectable-thematic svg');
    if (chartElement) {
  ...
  onPieChartKaeMouseOver(event: ChartMouseOverEvent) {
    const chartElement: HTMLElement | null = document.querySelector('.chart-selectable-kae svg');
    if (chartElement) {
      chartElement.style.cursor = 'pointer';
    }
  }
  ...
  applyFiltersThematic() {
    this.expensesPerThematicFiltered = this.expensesPerThematic.filter(item => {
  ...
  onAddToPersonalList(ada: string) {
    this.apiService.addDecisionToPersonalList(this.authService.getUserName()!, ada).subscribe({
      next: (data) => {
